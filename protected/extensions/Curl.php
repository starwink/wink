...
class Curl
{
    ...
    public $_status;
    private $_header, $_headerMap, $_error,  $_info;
    ...
    public function json($url, $data){
        $this->setHeaders(array('Content-Type: application/json','Content-Length: ' . strlen($data)));
    ...
    /**
     * [commonCurl curl提交只有get、post2种方式]
     * @param  [string] $action [控制器/方法]
     * @param  [array]  $param  [传值]
     * @param  [int]    $isPost [默认为0，get提交]
     * @return [array] [当返回为非array时，跳转回主页]
     */
    public function commonCurl($action,$params='',$isPost=0){
        $interfaceUrl = Yii::app()->params['interfaceUrl'].$action;//接口访问地址
        
        if (!$isPost) {
            $result = $this->get($interfaceUrl,$params);//搜索的所有下拉列表
        } else {
            $result = $this->post($interfaceUrl,$params);//搜索的所有下拉列表
        }
        $result = json_decode($result,true);//json转换、对象转数组

        if (!is_array($result)) {
            $this->redirect(Yii::app()->user->returnUrl);//跳转回主页
        }
